...
//get by name
const getOneGasStationByname = (req, res) => {
  const {name} = req.query;
  db.query(
    `SELECT * FROM gas_station WHERE name = ?`,
    [name],
    (error, result) => {
      if (error)
        return res
...
//get branch by name station
const getOneGasStationBranchBynameGasStation = (req, res) => {
  
  const {name} = req.query;
  db.query(
    `SELECT * FROM gas_station_branch gsb 
